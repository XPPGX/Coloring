...
void standardError(struct colorInfo* _result){
    ...
    double* numPerColorClass = (double*)calloc(_result->totalColorClassNum, sizeof(double));
    int nodeID = (_result->startWith == 0) ? 0 : 1;
    int color = -1;
    for(; nodeID < _result->NodeNum ; nodeID ++){
        color = _result->nodeColorArr[nodeID];
        numPerColorClass[color] ++;        
    }
    ...
    // 算Variance
    int colorIndex = (numPerColorClass[0] > 0) ? 1 : 0;
    for(; colorIndex < _result->totalColorClassNum ; colorIndex ++){
        classVarNodeNum += (numPerColorClass[colorIndex] - classAvgNodeNum) * (numPerColorClass[colorIndex] - classAvgNodeNum);
    }
    classVarNodeNum /= totalColorClassNum;

    // 算Standard Error(標準差, S.E.)
    double SE = sqrt(classVarNodeNum);
